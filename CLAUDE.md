...
```
CI runs all four (clippy is a gate; the local suite needs no display for most tests, and the `#[gtk::test]` widget/scope tests GTK-init against the live session locally and under `xvfb-run` in CI).
